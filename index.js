...
program
  .name('wx-format')
  .description('微信公众号 Markdown 排版工具 v1.4')
  .version('1.4.0')
  ...
  .action(async (input, options) => {
    ...
    try {
      const converter = new WxConverter({ theme, normalize: options.normalize, important: options.important });
...
program
  .command('topic')
  .description('公众号选题：聚合热点数据 + AI 分析')
  .option('--niche <keywords>', '领域关键词 (逗号分隔)')
  .option('--sources <list>', '数据源 (weibo,zhihu,baidu,36kr)', 'weibo,zhihu,baidu,36kr')
  .option('--count <n>', '显示条数', '20')
  .option('--analyze', '启用 AI 选题分析')
